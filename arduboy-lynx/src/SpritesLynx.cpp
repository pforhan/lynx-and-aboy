/**
 * @file SpritesLynx.cpp
 * \brief
 * Portable (C) implementation of the `Sprites` class for the Atari Lynx.
 *
 * The upstream `Sprites.cpp` draws into the RAM buffer using a large AVR
 * inline-assembly routine (GCC register constraints). On the Lynx we provide
 * the identical class/API using the same flow as the upstream `SpritesB.cpp`
 * ("optimized for small code size") implementation, so existing sketches that
 * call `Sprites::drawSelfMasked()`, etc. run unchanged.
 */
...
#if !defined(__AVR__)
// Lynx implementation of the `Sprites` class (see file header comment).
                         ...
                         uint8_t w, uint8_t h, uint8_t draw_mode)
{
  ...
  for (a = 0; a < loop_h; a++)
  {
    for (iCol = 0; iCol < rendered_width; iCol++)
    {
      uint8_t data;
